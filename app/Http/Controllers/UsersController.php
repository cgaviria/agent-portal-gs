...
class UsersController extends Controller
{
	...
	/**
	 * On  updating user account details
	 *
	 * @return Response
	 */
	...
	/**
	 * Get the page that displays Users listing.
	 *
	 * @return Response
	 */
	...
	/**
	 * Get the User listing in Admin Table
	 *
	 * @return Response
	 */
		...
		if ($user_check) {
			...
			if (!Sentinel::inRole(Role::ROLE_ADMIN)) {
				if ($logged_in_user->agency_id && Sentinel::inRole(Role::ROLE_AGENCY_MANAGER)) {
					$agent_role = Sentinel::findRoleBySlug(Role::ROLE_AGENT);
					$users->join('role_users', 'role_users.user_id', '=', 'users.id')
						  ->where('users.agency_id', $logged_in_user->agency_id)
					      ->where('role_users.role_id', $agent_role->id);
				} else if (Sentinel::inRole(Role::ROLE_OWNER)) {
					$agent_role = Sentinel::findRoleBySlug(Role::ROLE_AGENT);
					$agency_manager_role = Sentinel::findRoleBySlug(Role::ROLE_AGENCY_MANAGER);
					$users->leftjoin('agencies', 'agencies.id', '=', 'users.agency_id')
						  ->join('role_users', 'role_users.user_id', '=', 'users.id')
						  ->where('agencies.owner_id', $logged_in_user->id)
	 ...
	 * @return Response
	 */
	public function getDeactivateForm($id){
        
        $userL = Sentinel::check();        
        if($userL){
            $ci = User::find($id);
            return view('admin.user.deactivate',['ci'=>$ci])->render();
        }  
    }
	/**
	...
	public function deactivate(Request $request)
	{
        ...
        if($userL){
			...
			$this->insertActivity( "/dashboard/users/edit/".$request -> input('ci_id'),'deactivated an  <a href="%a" target="_blank">User</a>',$logged_in_user->id);
          	$response->mens = Lang::get('User successfully deactivated.');
          	return RestResponse::sendResult(200,$response);
        } 
	}
	/**
    ...
    /**
	 * Activate the user by modifying activation table.
	 *
	 * @return Response
	 */
